...
def test_gen_jwt_token_invalid_token_type():
    ...
    with pytest.raises(ValueError) as err:
        Token.gen_jwt_token(payload, token_type)
...
def test_gen_jwt_token_access_token():
    ...
    token_type = "access"
    access_token = Token.gen_jwt_token(payload, token_type)
    decoded_token = Token.decoded_jwt_token(access_token)
...
def test_gen_jwt_token_refresh_token():
    payload = {"id": 1, "email": "juren52@naver.com"}
    token_type = "refresh"
    refresh_token = Token.gen_jwt_token(payload, token_type)
    decoded_token = Token.decoded_jwt_token(refresh_token)
...
def test_decoded_jwt_token_with_error():
    ...
    with pytest.raises(AuthenticationFailed, match=re.escape("Signature verification failed")):
        Token.decoded_jwt_token(token[:-3] + 'xyz')
